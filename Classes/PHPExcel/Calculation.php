...
if (!defined('CALCULATION_REGEXP_CELLREF')) {
    ...
    if (defined('PREG_BAD_UTF8_ERROR')) {
        ...
        define('CALCULATION_REGEXP_CELLREF', '((([^\s,!&%^\/\*\+<>=-]*)|(\'[^\']*\')|(\"[^\"]*\"))!)?\$?\b([a-z]{1,3})\$?(\d{1,7})(?![\w.])'); # Owen added \b and (?![\w.])
        //    Named Range of cells
        define('CALCULATION_REGEXP_NAMEDRANGE', '((([^\s,!&%^\/\*\+<>=-]*)|(\'[^\']*\')|(\"[^\"]*\"))!)?([_A-Z][_A-Z0-9\.]*)');
    } else {
        //    Cell reference (cell or range of cells, with or without a sheet reference)
        define('CALCULATION_REGEXP_CELLREF', '(((\w*)|(\'[^\']*\')|(\"[^\"]*\"))!)?\$?\b([a-z]{1,3})\$?(\d+)(?![\w.])'); # Owen added \b and (?![\w.])
        //    Named Range of cells
...
//                    echo 'Element '.$val.' is a Cell reference<br />';
                    ...
                    if ($testPrevOp !== null && $testPrevOp['value'] == ':') { // Owen 20200630
...
//                    echo 'Element is a Variable, Constant, String, Number or Boolean<br />';
                    ...
                    $testPrevOp = $stack->last(1);
                    if ($testPrevOp !== null && $testPrevOp['value'] == ':') { // Owen 20200712
